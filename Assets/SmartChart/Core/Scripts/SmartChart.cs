...
namespace ToucanSystems
{
    ...
    public class SmartChart : MonoBehaviour
    {
        ...
        public bool showEnvirMarkers = true;
        [Range(1, 100)]
        ...
        private void DrawData()
        {
            if (chartData != null)
            {
                for (int i = 0; i < chartData.Length; i++)
                {
                    ...
                    if (showEnvirMarkers)
                    {
                        for (int j = 0; j < chartData[i].envirMarker.Length; j++)
                        {
                            int index = chartData[i].envirMarker[j].index;
                            float x = xMargin + (chartWidth - xMargin) * (chartData[i].data[index].x / (maxXValue - minXValue)) - (chartWidth - xMargin) * (minXValue / (maxXValue - minXValue));
                            float y = yMargin + (chartHeight - yMargin) * (chartData[i].data[index].y / (maxYValue - minYValue)) - (chartHeight - yMargin) * (minYValue / (maxYValue - minYValue));
                            Vector3 vec = new Vector3(x, y);
                            DrawMarker("envir_marker" + j + "_" + data.transform.name, chartData[i].envirMarker[j].value, vec, chartData[i].markerSprite, chartData[i].markerSize, chartData[i].markerColor);
                        }
                    }
            ...
            if (!showMarkers && !showEnvirMarkers)
            {
                DeleteMarkers(0);
            }
        }
